...
def test_the_token_is_not_what_is_stored() -> None:
    token = client.post("/auth/login", json={"email": OWNER, "password": PASSWORD}).json()["token"]
...
def test_another_merchants_run_is_not_reachable() -> None:
    ...
    run_id = client.post("/runs", json={"ticket_reference": "LU-1"}, headers=lumen_headers).json()[
        "id"
    ]
...
def test_only_one_run_at_a_time_per_ticket() -> None:
    ...
    headers = login()
    assert (
        client.post("/runs", json={"ticket_reference": "NW-2"}, headers=headers).status_code == 201
    )
...
def test_a_run_detail_carries_its_trajectory_and_its_bounds() -> None:
    headers = login()
    run_id = client.post("/runs", json={"ticket_reference": "NW-2"}, headers=headers).json()["id"]
    ...
    drive_run(
        ...
        ScriptedProvider(
            script=[
                [
                    call(
                        "issue_refund", order_reference="NW-1042", amount_cents=1900, reason="stale"
                    )
                ],
                text("done"),
            ]
...
def test_the_same_approval_cannot_be_decided_twice() -> None:
    ...
    first = client.post(
        ...
        json={"decision": "denied", "reason": "no"},
        headers=headers,
    )
...
def test_another_merchants_run_cannot_be_replayed() -> None:
    ...
    run_id = client.post("/runs", json={"ticket_reference": "LU-1"}, headers=lumen).json()["id"]
...
def test_an_id_that_cannot_be_an_id_is_a_404_not_a_500() -> None:
    ...
    assert client.post("/runs/nonsense/cancel", headers=headers).status_code == 404
...
def test_the_policy_permits_no_inline_or_third_party_script() -> None:
    ...
    directives = dict(
        (part.split(" ", 1) + [""])[:2] for part in (p.strip() for p in csp.split(";")) if part
    )
...
def test_starting_runs_is_throttled_per_merchant() -> None:
    ...
    seen = {
        client.post("/runs", json={"ticket_reference": "NW-1"}, headers=headers).status_code
        for _ in range(31)
    }
